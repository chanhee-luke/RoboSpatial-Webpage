# Main entry script to generate QA from indoor scan datasets
...
from qa_generator import generate_and_save_ann
...
def run(config):
    ...
    if embodiedscan_loader:
        ...
        num_total_obj_pairs = defaultdict(int)
        num_total_images = defaultdict(int)
        num_relationships_pairs = defaultdict(int)
        num_compatibility_pairs = defaultdict(int)
        ...
        for dataset_name, idx, scene_name in tqdm(embodiedscan_loader.list_scenes(dataset_list), desc=f"Scene loop\n"):
            ...
            completed_scenes = {}
            file_path = os.path.join((config[dataset_name]["output_path"]), f"progress.json")
            ...
            scene_stats = generate_and_save_ann(embodiedscan_loader, dataset_name, scene_name, image_ann_list, config, annotations=scene_obj_ann)
            
            num_total_obj_pairs[scene_stats["dataset_name"]] += scene_stats["num_total_obj_pairs"]
            num_total_images[scene_stats["dataset_name"]] += scene_stats["num_total_images"]
            num_relationships_pairs[scene_stats["dataset_name"]] += scene_stats["num_relationships_pairs"]
            num_compatibility_pairs[scene_stats["dataset_name"]] += scene_stats["num_compatibility_pairs"]
            ...
            sanitized_scene_name = scene_name.replace('/', '_')
            file_path = os.path.join((config[dataset_name]["output_path"]), f"{sanitized_scene_name}_ann_stats.json")
            with open(file_path, 'w') as file:
                json.dump(scene_stats, file, indent=4)
            
            # Save progress stats
            file_path = os.path.join((config[dataset_name]["output_path"]), f"{sanitized_scene_name}_progress.json")
            with open(file_path, 'w') as file:
                json.dump(completed_scenes[dataset_name], file, indent=4)
            
            print(f"Saved stats for {dataset_name} {scene_name}")
